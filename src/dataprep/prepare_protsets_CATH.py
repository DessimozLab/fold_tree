...
if __name__ == '__main__':
    ...
    def dlchain(pdb_id , chainID , savepath , unid):
        ...
        for i,fam in enumerate(siftsdf[category].unique()):
            ...
            if not os.path.exists(datapath+fam+ datapath+fam+'/sequences.fst'):
                ...
                if len(prots)> 100:
                    ...
                    pool = mp.Pool(8)
                    pool.starmap(dlchain, [ (protdict[unid], chaindict[unid] , datapath+fam+'/structs/'+unid.upper()+'.pdb' , unid.upper() ) for unid in unires_df['query'].unique() ] )
                    pool.close()
                    pool.join()
