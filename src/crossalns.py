...
def crossaln(aln1, aln2, outaln):
    #copy aln1 logic to aln2
    #aln1 and al2 are both file names

    #read in the alignments
    aln1 = [ rec for rec in Bio.SeqIO.parse(aln1, "fasta")]
    aln2 = [ rec for rec in  Bio.SeqIO.parse(aln2, "fasta") ]

    #get the sequences by removing the gaps
    seqs1= { rec.id: str(rec.seq) for rec in aln1 }
    seqs2= { rec.id: str(rec.seq).replace("-","") for rec in aln2 }
